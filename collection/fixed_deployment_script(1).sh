#!/bin/bash
# fixed_deployment.sh - Enhanced automated deployment script for Gemini Claude Adapter

set -euo pipefail # Exit on error, undefined variables, and pipe failures

# --- Configuration ---
readonly APP_USER="gemini"
readonly APP_DIR="/home/$APP_USER/gemini-claude-adapter"
...
check_system() {
    ...
    if [[ ! -f "main.py" && ! -f "fixed_main_py(1).py" && ! -f "fixed_main_py.py" ]]; then
...
setup_app_environment() {
    ...
    # Handle main.py file naming
    if [[ -f "$APP_DIR/fixed_main_py(1).py" ]]; then
        mv "$APP_DIR/fixed_main_py(1).py" "$APP_DIR/main.py"
        log_info "Renamed fixed_main_py(1).py to main.py"
    elif [[ -f "$APP_DIR/fixed_main_py.py" ]]; then
        mv "$APP_DIR/fixed_main_py.py" "$APP_DIR/main.py"
        log_info "Renamed fixed_main_py.py to main.py"
    elif [[ ! -f "$APP_DIR/main.py" ]]; then
        log_error "No main.py file found after copying"
    fi
...
command=$APP_DIR/venv/bin/uvicorn main:app --host 0.0.0.0 --port 8000 --workers 1
directory=$APP_DIR
user=$APP_USER
autostart=true
autorestart=true
...
APP_DIR="/home/gemini/gemini-claude-adapter"
APP_USER="gemini"
...
" 2>/dev/null || log_warning "Health check timeout or failed"
    ...
    error-logs)
        ...
        ;;
    monitor)
        if [[ -f "$APP_DIR/monitoring_scripts.py" ]]; then
            log_info "Starting monitoring interface..."
            cd "$APP_DIR" && sudo -u "$APP_USER" ./venv/bin/python monitoring_scripts.py monitor
        else
            log_error "Monitoring script not found at $APP_DIR/monitoring_scripts.py"
            exit 1
        fi
        ;;
    update)
    ...
    *) 
        ...
        echo "Usage: $0 {start|stop|restart|status|logs|error-logs|monitor|update|backup}"
        echo ""
        echo "Commands:"
        ...
        echo "  monitor     - Start real-time monitoring interface"
